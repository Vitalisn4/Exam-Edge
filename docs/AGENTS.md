...
12. `docs/context/progress-tracker.md` — current phase, completed work, next steps
13. `docs/context/specs/` — per-unit spec files (`00-spec-template.md`)
14. `docs/context/documentation-map.md` — single source of truth index
...
- Offline: expo-sqlite (local question cache), IndexedDB via react-native-mmkv
...
Start simulation → Fullscreen mode
  ...
  → Return template report immediately (SQL aggregates)
  → Async: Haiku report_gen narrative (~30s deferred) — see ai-cost-and-exam-system.md §9
