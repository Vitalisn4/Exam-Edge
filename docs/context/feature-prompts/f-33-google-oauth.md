...
|-------|-------|
| **Feature ID** | F-33 |
| **Feature name** | Google OAuth |
| **Release** | V1.1 |
| **Branch** | `feature/f-33-google-oauth` |
| **Prerequisites** | Unit 06 merged to main. |

## Purpose and business objectives

Auth.js Google provider alongside email/password

## Problem statement

Post-MVP expansion requires **Google OAuth** to scale ExamEdge beyond the Cameroon MVP pilot per `roadmap.md`.

## User value

Google OAuth unlocks the roadmap milestone for **V1.1**, extending access, security, or platform capabilities for students, teachers, and institutional partners.

## User stories

- As a platform stakeholder, I need google oauth so ExamEdge can expand per roadmap.md without breaking MVP invariants.
- As a developer, I implement google oauth on a dedicated branch with full test coverage and documentation updates.
- As a student or teacher (where applicable), I benefit from google oauth without regression to core marking, hints, or offline study flows.

## Acceptance criteria

- [ ] Google login → dashboard; account linking strategy documented
...
- [ ] `security.md` · `library-docs.md` Auth.js · `roadmap.md`
...
Implement Google OAuth per roadmap.md and architecture.md.
...
Optional: `accounts` table for Auth.js adapter
...
OAuth state validation; role defaults to student
...
4. Create branch `feature/f-33-google-oauth`.
