# F-47: Whisper.cpp Oral Assessment
...
|-------|-------|
| **Feature ID** | F-47 |
| **Feature name** | Whisper.cpp Oral Assessment |
| **Release** | V2.0 |
| **Branch** | `feature/f-47-whisper-oral-asr` |
| **Prerequisites** | F-32 mobile app complete. |

## Purpose and business objectives

On-device ASR for oral explanation probes

## Problem statement

Post-MVP expansion requires **Whisper.cpp Oral Assessment** to scale ExamEdge beyond the Cameroon MVP pilot per `roadmap.md`.

## User value

Whisper.cpp Oral Assessment unlocks the roadmap milestone for **V2.0**, extending access, security, or platform capabilities for students, teachers, and institutional partners.

## User stories

- As a platform stakeholder, I need whisper.cpp oral assessment so ExamEdge can expand per roadmap.md without breaking MVP invariants.
- As a developer, I implement whisper.cpp oral assessment on a dedicated branch with full test coverage and documentation updates.
- As a student or teacher (where applicable), I benefit from whisper.cpp oral assessment without regression to core marking, hints, or offline study flows.

## Acceptance criteria

- [ ] Record → transcribe → evaluate oral UVE response
...
- [ ] `AGENTS.md` · `platform-ecosystem-ops.md` · mobile F-32
...
Implement Whisper.cpp Oral Assessment per roadmap.md and architecture.md.
...
4. Create branch `feature/f-47-whisper-oral-asr`.
