...
|-------|-------|
| **Feature ID** | F-42 |
| **Feature name** | WAEC / NECO Boards |
| **Release** | V2.0 |
| **Branch** | `feature/f-42-waec-neco` |
| **Prerequisites** | V1.1 content architecture complete. |

## Purpose and business objectives

Nigeria/Ghana board configs + validated question pools

## Problem statement

Post-MVP expansion requires **WAEC / NECO Boards** to scale ExamEdge beyond the Cameroon MVP pilot per `roadmap.md`.

## User value

WAEC / NECO Boards unlocks the roadmap milestone for **V2.0**, extending access, security, or platform capabilities for students, teachers, and institutional partners.

## User stories

- As a platform stakeholder, I need waec / neco boards so ExamEdge can expand per roadmap.md without breaking MVP invariants.
- As a developer, I implement waec / neco boards on a dedicated branch with full test coverage and documentation updates.
- As a student or teacher (where applicable), I benefit from waec / neco boards without regression to core marking, hints, or offline study flows.

## Acceptance criteria

- [ ] WAEC marking benchmarks ≥92%; board-specific mark scheme templates
...
- [ ] `content-architecture.md` · `content-governance.md` · `boards.ts`
...
Implement WAEC / NECO Boards per roadmap.md and architecture.md.
...
4. Create branch `feature/f-42-waec-neco`.
...
New AI chains — config + content only
