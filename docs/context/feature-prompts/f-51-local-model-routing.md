...
| **Release** | V2.0+ |
| **Branch** | `feature/f-51-local-model-routing` |
| **Prerequisites** | Unit 09 router complete; marking benchmarks ≥92% established. |

## Purpose and business objectives

Fine-tuned Haiku-class models for marking via router swap

## Problem statement

Post-MVP expansion requires **Local Model Routing** to scale ExamEdge beyond the Cameroon MVP pilot per `roadmap.md`.

## User value

Local Model Routing unlocks the roadmap milestone for **V2.0+**, extending access, security, or platform capabilities for students, teachers, and institutional partners.

## User stories

- As a platform stakeholder, I need local model routing so ExamEdge can expand per roadmap.md without breaking MVP invariants.
- As a developer, I implement local model routing on a dedicated branch with full test coverage and documentation updates.
- As a student or teacher (where applicable), I benefit from local model routing without regression to core marking, hints, or offline study flows.

## Acceptance criteria

- [ ] `getModelConfig()` checks local registry first; benchmark ≥92% before rollout

## Prerequisites and dependencies

Unit 09 router complete; marking benchmarks ≥92% established.
...
- [ ] `AGENTS.md` § Model Router · `ai-cost-and-exam-system.md`
...
Implement Local Model Routing per roadmap.md and architecture.md.
...
4. Create branch `feature/f-51-local-model-routing`.
