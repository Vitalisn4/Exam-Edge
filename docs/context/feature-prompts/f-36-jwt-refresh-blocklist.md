# F-36: JWT Refresh Rotation + Token Blocklist
...
|-------|-------|
| **Feature ID** | F-36 |
| **Feature name** | JWT Refresh Rotation + Token Blocklist |
| **Release** | V1.1 |
| **Branch** | `feature/f-36-jwt-refresh-blocklist` |
...
Post-MVP expansion requires **JWT Refresh Rotation + Token Blocklist** to scale ExamEdge beyond the Cameroon MVP pilot per `roadmap.md`.

## User value

JWT Refresh Rotation + Token Blocklist unlocks the roadmap milestone for **V1.1**, extending access, security, or platform capabilities for students, teachers, and institutional partners.

## User stories

- As a platform stakeholder, I need jwt refresh rotation + token blocklist so ExamEdge can expand per roadmap.md without breaking MVP invariants.
- As a developer, I implement jwt refresh rotation + token blocklist on a dedicated branch with full test coverage and documentation updates.
- As a student or teacher (where applicable), I benefit from jwt refresh rotation + token blocklist without regression to core marking, hints, or offline study flows.

## Acceptance criteria

- [ ] Refresh token rotation; logout invalidates refresh; stolen token window minimized
...
Implement JWT Refresh Rotation + Token Blocklist per roadmap.md and architecture.md.
...
4. Create branch `feature/f-36-jwt-refresh-blocklist`.
