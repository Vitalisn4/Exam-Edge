...
|-------|-------|
| **Feature ID** | F-35 |
| **Feature name** | PostgreSQL RLS Policies |
| **Release** | V1.1 |
| **Branch** | `feature/f-35-postgresql-rls` |
| **Prerequisites** | Units 05 and 06 merged to main. |

## Purpose and business objectives

Row-level security per role as defense-in-depth

## Problem statement

Post-MVP expansion requires **PostgreSQL RLS Policies** to scale ExamEdge beyond the Cameroon MVP pilot per `roadmap.md`.

## User value

PostgreSQL RLS Policies unlocks the roadmap milestone for **V1.1**, extending access, security, or platform capabilities for students, teachers, and institutional partners.

## User stories

- As a platform stakeholder, I need postgresql rls policies so ExamEdge can expand per roadmap.md without breaking MVP invariants.
- As a developer, I implement postgresql rls policies on a dedicated branch with full test coverage and documentation updates.
- As a student or teacher (where applicable), I benefit from postgresql rls policies without regression to core marking, hints, or offline study flows.

## Acceptance criteria

- [ ] Student JWT can only SELECT own rows; teacher sees assigned class; admin sees validation queue

## Prerequisites and dependencies

Units 05 and 06 merged to main.
...
- [ ] `security.md` deferred items · `roadmap.md` · `ExamEdge_Engineering_Document.md` §5
...
Implement PostgreSQL RLS Policies per roadmap.md and architecture.md.
...
4. Create branch `feature/f-35-postgresql-rls`.
