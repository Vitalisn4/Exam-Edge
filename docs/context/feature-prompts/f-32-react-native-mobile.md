# F-32: React Native + Expo Mobile App
...
|-------|-------|
| **Feature ID** | F-32 |
| **Feature name** | React Native + Expo Mobile App |
| **Release** | V1.1 |
| **Branch** | `feature/f-32-react-native-mobile` |
| **Prerequisites** | All MVP Units 01–31 merged to main. |

## Purpose and business objectives

Native mobile app sharing web API; offline SQLite question cache

## Problem statement

Post-MVP expansion requires **React Native + Expo Mobile App** to scale ExamEdge beyond the Cameroon MVP pilot per `roadmap.md`.

## User value

React Native + Expo Mobile App unlocks the roadmap milestone for **V1.1**, extending access, security, or platform capabilities for students, teachers, and institutional partners.

## User stories

- As a platform stakeholder, I need react native + expo mobile app so ExamEdge can expand per roadmap.md without breaking MVP invariants.
- As a developer, I implement react native + expo mobile app on a dedicated branch with full test coverage and documentation updates.
- As a student or teacher (where applicable), I benefit from react native + expo mobile app without regression to core marking, hints, or offline study flows.

## Acceptance criteria

- [ ] Login → practice → submit → marks on Android 7+ device; offline cache serves cached questions

## Prerequisites and dependencies

All MVP Units 01–31 merged to main.
...
- [ ] `roadmap.md` V1.1 · `architecture.md` mobile note · `platform-ecosystem-ops.md` · `library-docs.md` Expo
...
`apps/mobile/` Expo Router; secure token in expo-secure-store; same API as web
...
JWT in secure store — not AsyncStorage; same RBAC as web
...
4. Create branch `feature/f-32-react-native-mobile`.
...
Rewriting backend; new AI chains
