...
|-------|-------|
| **Feature ID** | Unit 08 |
| **Feature name** | Redis + Rate Limiting |
| **Release** | V1.0 MVP |
| **Branch** | `feature/unit-08-redis-rate-limiting` |
...
This unit is required to deliver **Redis + Rate Limiting** as defined in `build-plan.md` Unit 08. Without it, the MVP pilot cannot proceed to the next build phase.
...
- [ ] Idempotency SET NX behavior (mock Redis)
...
4. Create branch `feature/unit-08-redis-rate-limiting`.
